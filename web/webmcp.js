...
(() => {
  ...
  const learningTool = {name:"get_learning_summary",title:"Read workspace improvement evidence",description:"Read feedback counts and saved presets from the connected gateway, or local feedback and model status when disconnected. Does not train or promote a model.",inputSchema:{type:"object",properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:withActivity("get_learning_summary",()=>window.BeatForgeLearning.getSummary())};
  const feedbackTool = {name:"record_mapping_feedback",title:"Record feedback supplied by a person",description:"Save a person's explicitly supplied mapping preference for an existing chart in the currently connected workspace. Never infer or invent a rating. This does not record a headset clear.",inputSchema:{type:"object",properties:{jobId:{type:"string",pattern:"^(?:[a-f0-9]{12}|[a-f0-9]{32})$"},difficulty:{type:"string",enum:difficulties},tester:{type:"string",minLength:1,maxLength:120},notes:{type:"string",maxLength:2000},tags:{type:"array",items:{type:"string",enum:["too_dense","too_sparse","awkward","tiring","repetitive","off_beat","good_flow"]}},ratings:{type:"object",additionalProperties:false,properties:Object.fromEntries(["overall","flow","readability","musicality","variety"].map(name=>[name,{type:"integer",minimum:1,maximum:5}]))}},required:["jobId","difficulty","tester"],additionalProperties:false},annotations:{readOnlyHint:false,destructiveHint:false},execute:withActivity("record_mapping_feedback",input=>window.BeatForgeLearning.recordFeedback(input))};
