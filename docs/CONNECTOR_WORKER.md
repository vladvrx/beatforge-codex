...
2. Send raw audio bytes to the returned `uploadPath` using `PUT` and the account's authorization header. Audio is not embedded in MCP messages. Each audio transfer has a five-minute deadline; HTTP 408 removes partial bytes and releases the writer so the same unexpired reservation can be retried. Use `get_upload` or `GET /api/uploads/{id}` to confirm `ready` and its SHA-256 hash.
...
An idempotency key can be retried with the same normalized request. Reusing it for a different request fails. At most two queued/running jobs per account are allowed. Uploads have a 64 MiB per-file limit, 256 MiB reserved account storage limit and 32 reservations per account. An unfinished reservation expires after one hour. Expired pending reservations are reclaimed atomically when a new upload is reserved. Ready uploads and writing reservations remain counted and preserved; retention of completed data and recovery after a process crash still require operator action. The gateway validates extensions and byte counts; the local decoder validates actual audio contents.
